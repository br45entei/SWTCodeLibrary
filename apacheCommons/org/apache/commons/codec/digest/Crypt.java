...
public class Crypt {
	 ...
	 * algorithm!
	 * </ul>
	 * The magic strings {@code "$apr1$"} and {@code "$2a$"} are not recognized
	 * by this method as its output should be
	 ...
	 * </pre>
	 * <p>
	 * The resulting string starts with the marker string ({@code $6$}),
	 * continues with the salt value and ends with a
	 * {@code "$"} sign followed by the actual hash value. For DES the string
	 * only contains the salt and actual hash.
