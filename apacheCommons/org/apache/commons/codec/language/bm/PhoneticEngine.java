 ...
 * into account the likely source language. Next, this phonetic representation
 * is converted into a
 * pan-European 'average' representation, allowing comparison between different
 * versions of essentially
...
public class PhoneticEngine {
	...
	static final class PhonemeBuilder {
		 ...
		 * phoneme of zero characters. This can then be appended to. This should
		 * be the only way to create a new
		...
		protected PhonemeBuilder(final Set<Rule.Phoneme> phonemes) {
		...
		/** Applies the given phoneme expression to all phonemes in this phoneme
		 * builder.
		 * <p>
		 * This will lengthen phonemes that have compatible language sets to the
		 * expression, and drop those that are
		 * incompatible.
		 *
		 ...
		 * strings of each phoneme,
		 * joined with a pipe. This is explicitly provided in place of toString
		 * as it is a potentially
	 ...
	 * index of the next char in <code>input</code> that must be processed next
	...
	private static final class RulesApplication {
		 ...
		 * and pattern. Then applies this rule to the phoneme builder to produce
		 * updated phonemes. If there was no
	...
	private PhonemeBuilder applyFinalRules(final PhonemeBuilder phonemeBuilder, final Map<String, List<Rule>> finalRules) {
		...
		for(final Rule.Phoneme phoneme : phonemeBuilder.getPhonemes()) {
			...
			for(int i = 0; i < phonemeText.length();) {
				final RulesApplication rulesApplication = new RulesApplication(finalRules, phonemeText, subBuilder, i, this.maxPhonemes).invoke();
	 ...
	 *         '-'-separated phonetic representations of the
	 *         input */
	public String encode(String input, final Languages.LanguageSet languageSet) {
		...
		for(int i = 0; i < input.length();) {
			final RulesApplication rulesApplication = new RulesApplication(rules, input, phonemeBuilder, i, this.maxPhonemes).invoke();
			i = rulesApplication.getI();
	...
	/** Gets if multiple phonetic encodings are concatenated or if just the
	 * first one is kept.
	 *
